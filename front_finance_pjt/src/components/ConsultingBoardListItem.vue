        ...
        <v-list-item-title class="text-h5 mb-2">
          <span @click.prevent="navigateToDetailView">
            <a href="" class="text-decoration-none">
              {{ board.title }} [{{ board.answer_set.length }}]
            </a>
          </span>
        </v-list-item-title>
        <v-list-item-subtitle>글쓴이: {{ board.username }} | 작성시간: {{ formatDateTime(board.created_at) }}</v-list-item-subtitle>
      </v-list-item-content>
      <v-divider></v-divider>
    </v-list-item>
  </v-list>
...
const navigateToDetailView = () => {
  ...
  router.push({ name: 'ConsultingBoardDetailView', params: { id: props.board.id } })
}
</script>
